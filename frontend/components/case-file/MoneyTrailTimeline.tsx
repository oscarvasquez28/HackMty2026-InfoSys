...
// destination does not match the next step's source.
...
export const MoneyTrailTimeline: React.FC<MoneyTrailTimelineProps> = ({ trail }) => {
  return (
    <ol>
      {trail.map((step, index) => (
        <li key={`${step.stepOrder}-${step.step.exhibit_id}`} className="case-avoid-break grid grid-cols-[auto_1fr] gap-4">
          <div className="flex flex-col items-center">
            <div className="grid h-7 w-7 shrink-0 place-items-center rounded-full border-2 border-paper-ink font-mono text-xs">
              {step.stepOrder}
            </div>
            {index < trail.length - 1 && <div className="mt-1 w-px flex-1 bg-paper-border" />}
          </div>
          <div className="pb-4">
