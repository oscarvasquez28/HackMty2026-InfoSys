...
| **Data Estate page (`/investigate/data`): SQLite/CSV/CFDI-XML/JSON ingestion in the browser** | [`doc/frontend/README.md`](./frontend/README.md) | §11: `sql.js` WASM, per-table parsers, estate-wide validation, checking exhibits against the estate, `estate.db` export. | Backend Python code or the seeder doc below |
| **Audit history endpoints (`/api/v1/reports`), `audit_reports` table, or report persistence** | [`doc/backend/api/README.md`](./backend/api/README.md) | §4.6: reports router, `persist_audit_report` contract, `database_enabled` degradation. | Frontend components or graph algorithms |
| **Audit History screen (`/investigate/history`): listing and reopening past audit reports** | [`doc/frontend/README.md`](./frontend/README.md) | §12: `useAuditReports` hook, `ReportHistoryScreen`, reopening reports in the Case File Viewer. | Backend Python services |
